...
```
-A INPUT -s 10.254.140.97/32 -p tcp -m state --state NEW -m tcp -m comment --comment "Khai bao Composable" -j ACCEPT
-A INPUT -s 10.254.140.98/32 -p tcp -m state --state NEW -m tcp -m comment --comment "Khai bao Composable" -j ACCEPT
-A INPUT -s 10.254.140.99/32 -p tcp -m state --state NEW -m tcp -m comment --comment "Khai bao Composable" -j ACCEPT
```
...
Tham khảo file mẫu đính kèm:

![refer1](../refer/nifi.properties) 

#### 1.2.2. Chỉnh sửa file state-managent.xml
...
`home/nifi/bin/nifi.sh set-single-user-credentials nifi AdminVt@1234`

Trong đó: 

-	`nifi` là username

-	`AdminVt@1234` là password
